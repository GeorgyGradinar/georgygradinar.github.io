...
import {Properties} from '@/constants/properties.const'
...
export default {
  ...
  data() {
    return {
      isBlackTheme: true,
      isLanguageUk: true,
      toggleLanguageButtonSkill: Translation.skill.uk,
      toggleLanguageButtonProject: 'Project',
    }
  },
  ...
  methods: {
    toggleTheme() {
      ...
      const key = this.isBlackTheme ? 'dark' : 'light';
      Properties.forEach((item)=>{
        document.documentElement.style.setProperty(item.property, item.value[key]);
      })
    },

    toggleLanguage() {
      this.isLanguageUk = !this.isLanguageUk;
      this.toggleLanguageButtonSkill = this.isLanguageUk ? Translation.skill.uk : Translation.skill.ru;
      this.toggleLanguageButtonProject = this.isLanguageUk ? Translation.project.uk : Translation.project.ru;
      this.$emit('language', this.isLanguageUk);
    },
...
.change-property:hover {
  transform: scale(1.2);
}

.change-property:active{
  margin-bottom: 5px;
}
