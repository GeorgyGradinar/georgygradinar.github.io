<template>

  <scroll-bar></scroll-bar>

  <div class="arrow" v-if="this.scroll" @click="topScrollTo"></div>

  <div class="wrap">

    <block_header @language="toggleLanguage"></block_header>

    <div class="wrap-first-show">

      <show_cart :language="languageUk"></show_cart>

      <div class="about-me">

        <h1 class="title">Junior Front-end Developer</h1>
        <h5 class="text">{{ this.toggleLanguageText }}</h5>
      </div>

      <div class="social">

        <svg class="icon" viewBox="0 0 30 30"
             @click="routerURL('https://github.com/GeorgyGradinar')">
          <path
              d="M15,3C8.373,3,3,8.373,3,15c0,5.623,3.872,10.328,9.092,11.63C12.036,26.468,12,26.28,12,26.047v-2.051 c-0.487,0-1.303,0-1.508,0c-0.821,0-1.551-0.353-1.905-1.009c-0.393-0.729-0.461-1.844-1.435-2.526 c-0.289-0.227-0.069-0.486,0.264-0.451c0.615,0.174,1.125,0.596,1.605,1.222c0.478,0.627,0.703,0.769,1.596,0.769 c0.433,0,1.081-0.025,1.691-0.121c0.328-0.833,0.895-1.6,1.588-1.962c-3.996-0.411-5.903-2.399-5.903-5.098 c0-1.162,0.495-2.286,1.336-3.233C9.053,10.647,8.706,8.73,9.435,8c1.798,0,2.885,1.166,3.146,1.481C13.477,9.174,14.461,9,15.495,9 c1.036,0,2.024,0.174,2.922,0.483C18.675,9.17,19.763,8,21.565,8c0.732,0.731,0.381,2.656,0.102,3.594 c0.836,0.945,1.328,2.066,1.328,3.226c0,2.697-1.904,4.684-5.894,5.097C18.199,20.49,19,22.1,19,23.313v2.734 c0,0.104-0.023,0.179-0.035,0.268C23.641,24.676,27,20.236,27,15C27,8.373,21.627,3,15,3z"/>
        </svg>

        <svg class="icon" viewBox="0 0 64 64"
             @click="routerURL('https://www.facebook.com/profile.php?id=100077375043055')">
          <path
              d="M32,6C17.642,6,6,17.642,6,32c0,13.035,9.603,23.799,22.113,25.679V38.89H21.68v-6.834h6.433v-4.548	c0-7.529,3.668-10.833,9.926-10.833c2.996,0,4.583,0.223,5.332,0.323v5.965h-4.268c-2.656,0-3.584,2.52-3.584,5.358v3.735h7.785	l-1.055,6.834h-6.73v18.843C48.209,56.013,58,45.163,58,32C58,17.642,46.359,6,32,6z"/>
        </svg>

        <svg class="icon" viewBox="0 0 24 24"
             @click="routerURL('https://www.linkedin.com/in/georgy-gradinar-194341245/')">
          <path
              d="M19,3H5C3.895,3,3,3.895,3,5v14c0,1.105,0.895,2,2,2h14c1.105,0,2-0.895,2-2V5C21,3.895,20.105,3,19,3z M9,17H6.477v-7H9V17z M7.694,8.717c-0.771,0-1.286-0.514-1.286-1.2s0.514-1.2,1.371-1.2c0.771,0,1.286,0.514,1.286,1.2S8.551,8.717,7.694,8.717z M18,17h-2.442v-3.826c0-1.058-0.651-1.302-0.895-1.302s-1.058,0.163-1.058,1.302c0,0.163,0,3.826,0,3.826h-2.523v-7h2.523v0.977C13.93,10.407,14.581,10,15.802,10C17.023,10,18,10.977,18,13.174V17z"/>
        </svg>

        <svg class="icon" viewBox="0 0 24 24"
             @click="routerURL('https://t.me/Georgy_Gradinar')">
          <path
              d="M 20.572266 3.0117188 C 20.239891 2.9764687 19.878625 3.028375 19.515625 3.171875 C 19.065625 3.348875 12.014406 6.3150313 5.4414062 9.0820312 L 3.2695312 9.9960938 C 2.4285313 10.337094 2.0039062 10.891672 2.0039062 11.638672 C 2.0039062 12.161672 2.22525 12.871063 3.28125 13.289062 L 6.9472656 14.757812 C 7.2642656 15.708813 8.0005469 17.916906 8.1855469 18.503906 C 8.2955469 18.851906 8.5733906 19.728594 9.2753906 19.933594 C 9.4193906 19.982594 9.5696563 20.007813 9.7226562 20.007812 C 10.165656 20.007812 10.484625 19.801641 10.640625 19.681641 L 12.970703 17.710938 L 15.800781 20.328125 C 15.909781 20.439125 16.486719 21 17.261719 21 C 18.228719 21 18.962234 20.195016 19.115234 19.416016 C 19.198234 18.989016 21.927734 5.2870625 21.927734 5.2890625 C 22.172734 4.1900625 21.732219 3.6199531 21.449219 3.3769531 C 21.206719 3.1694531 20.904641 3.0469688 20.572266 3.0117188 z M 19.910156 5.171875 C 19.533156 7.061875 17.478016 17.378234 17.166016 18.865234 L 13.029297 15.039062 L 10.222656 17.416016 L 11 14.375 C 11 14.375 16.362547 8.9468594 16.685547 8.6308594 C 16.945547 8.3778594 17 8.2891719 17 8.2011719 C 17 8.0841719 16.939781 8 16.800781 8 C 16.675781 8 16.506016 8.1197812 16.416016 8.1757812 C 15.272669 8.8885973 10.404094 11.662239 8.0078125 13.025391 L 4.53125 11.636719 L 6.21875 10.927734 C 10.51775 9.1177344 18.174156 5.893875 19.910156 5.171875 z"/>
        </svg>

        <svg class="icon" viewBox="0 0 24 24"
             @click="routerURL('https://twitter.com/Georgy72806971')">
          <path
              d="M22,3.999c-0.78,0.463-2.345,1.094-3.265,1.276c-0.027,0.007-0.049,0.016-0.075,0.023c-0.813-0.802-1.927-1.299-3.16-1.299c-2.485,0-4.5,2.015-4.5,4.5c0,0.131-0.011,0.372,0,0.5c-3.353,0-5.905-1.756-7.735-4c-0.199,0.5-0.286,1.29-0.286,2.032c0,1.401,1.095,2.777,2.8,3.63c-0.314,0.081-0.66,0.139-1.02,0.139c-0.581,0-1.196-0.153-1.759-0.617c0,0.017,0,0.033,0,0.051c0,1.958,2.078,3.291,3.926,3.662c-0.375,0.221-1.131,0.243-1.5,0.243c-0.26,0-1.18-0.119-1.426-0.165c0.514,1.605,2.368,2.507,4.135,2.539c-1.382,1.084-2.341,1.486-5.171,1.486H2C3.788,19.145,6.065,20,8.347,20C15.777,20,20,14.337,20,8.999c0-0.086-0.002-0.266-0.005-0.447C19.995,8.534,20,8.517,20,8.499c0-0.027-0.008-0.053-0.008-0.08c-0.003-0.136-0.006-0.263-0.009-0.329c0.79-0.57,1.475-1.281,2.017-2.091c-0.725,0.322-1.503,0.538-2.32,0.636C20.514,6.135,21.699,4.943,22,3.999z"/>
        </svg>

      </div>

    </div>

    <cv :language="languageUk"></cv>

    <skills :language="languageUk"></skills>

    <project :language="languageUk"></project>

    <p class="footer">© created by Gradinar Georgy</p>

  </div>
</template>

<script>
import block_header from './components/block-header'
import show_cart from './components/show-cart'
import skills from "@/components/skills";
import project from "@/components/project";
import cv from "@/components/download-cv";
import scrollBar from "@/components/scrollBar";

export default {
  name: 'App',
  components: {
    block_header,
    show_cart,
    skills,
    project,
    cv,
    scrollBar,
  },

  data() {
    return {
      languageUk: true,
      toggleLanguageText: 'I’m junior developer. I graduated BMSTU.' +
          ' I left logistics and decided to change my job. I chose web development because there are many opportunities for growth in this industry and' +
          ' I’m also interested in web applications.',
      scroll: false,
    }
  },

  mounted() {
    document.addEventListener('scroll', () => {
      this.scrollFunction()
    })
  },

  methods: {

    scrollFunction() {
      this.scroll = document.body.scrollTop > 100 || document.documentElement.scrollTop > 100;
    },

    topScrollTo() {
      window.scrollTo({
        top: 0, behavior: "smooth"
      })
    },

    toggleLanguage(language) {
      this.languageUk = language;
      this.toggleLanguageText = this.languageUk ?
          'I’m junior developer. I graduated BMSTU.' +
          ' I left logistics and decided to change my job. I chose web development because there are many opportunities for growth in this industry and' +
          ' I’m also interested in web applications.' :
          '  Закончил МГТУ им. Н. Э. Баумана. После 5 лет работы в логистике, решил сменить сферу и выбрал web development,' +
          ' так как еще в годы университета интересовался веб приложениями. Обучаюсь более полугода в этом направ-лении. ';
    }
    ,

    routerURL(url) {
      window.open(url, '_blank');
    },

  }
}
</script>

<style>

:root {
  --background: #202124;
  --color: #CAD2C5;
  --hover: #84A98C;
  --box-shadow: 5px -5px 10px #131416, -5px 5px 10px #2d2e32;
  --box-shadow-hover: 3px -3px 9px #354f52, -3px 3px 9px #52796f;
  --scrollGradient: linear-gradient(180deg, rgba(32,33,36,1) 0%, rgba(202,210,197,0.3) 50%, rgba(32,33,36,1) 100%);
}

* {
  margin: 0;
  padding: 0;
  box-sizing: border-box;
  font-family: 'Open Sans', sans-serif;
  cursor: default;
}

body {
  background-color: var(--background);
  color: var(--color);
  -ms-overflow-style: none;
}

body::-webkit-scrollbar {
  width: 0;
}

.arrow,
.arrow:before {
  position: fixed;
  top: 90%;
  right: 10%;
}

.arrow {
  width: 30px;
  height: 30px;
  margin: 20px 0 0 20px;
  -webkit-transform: rotate(225deg);
  border-left: none;
  border-top: none;
  border-right: 2px var(--color) solid;
  border-bottom: 2px var(--color) solid;
  cursor: pointer;
}

.arrow:before {
  content: "";
  width: 20px;
  height: 20px;
  top: 50%;
  margin: -10px 0 0 0;
  border-left: none;
  border-top: none;
  border-right: 1px var(--color) solid;
  border-bottom: 1px var(--color) solid;
  animation-duration: 3s;
  animation-iteration-count: infinite;
  animation-name: arrow;
}

@keyframes arrow {
  0% {
    opacity: 1;
  }
  100% {
    opacity: 0;
    transform: translate(20px, 20px);
  }
}

.wrap,
.about-me,
.wrap-first-show {
  display: flex;
  flex-direction: column;
  align-items: center;
}

.wrap-first-show {
  height: calc(100vh - 70px);
  padding-bottom: 50px;
}

.about-me {
  margin-bottom: 50px;
}

.title,
.text {
  display: inline-block;
}

.title {
  font-weight: 700;
  font-size: 60px;
  line-height: 100px;
}

.text {
  display: flex;
  align-items: center;
  text-align: center;
  width: 600px;
  height: 250px;
  font-weight: 500;
  font-size: 25px;
  line-height: 40px;
}

.social {
  display: flex;
  gap: 30px;
  margin-bottom: 50px;
}

.social svg {
  fill: var(--color);
  width: 40px;
  height: 40px;
  transition: all 0.2s;
}

.social svg,
.social svg path {
  cursor: pointer;
}

.social svg:hover {
  transform: scale(1.2);
  text-shadow: var(--box-shadow);
}

.footer {
  margin-bottom: 50px;
}

@media screen and (max-width: 900px) {
  * {
    transition: all 1s;
  }

  .title {
    font-size: 25px;
  }

  .text {
    width: 300px;
    font-size: 15px;
  }
}

</style>
